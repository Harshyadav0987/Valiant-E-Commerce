...
function Collection() {
    const {products, search, ShowSearch, productsLoaded} = useContext(ValiantContext);
    ...
    useEffect(() => {
        // ✅ Only apply filters when products are loaded
        if(productsLoaded) {
            applyFilter();
        }
    }, [category, subCategory, search, products, productsLoaded])

    useEffect(() => {
        if(productsLoaded) {
            sortProduct();
        }
    }, [sortType, productsLoaded])
